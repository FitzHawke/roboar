...
class Song:
    def __init__(self, name, path, artist, album, destination):
        ...
        self.dest_path = os.path.join(
            ...
            self.artist,
            self.album,
            self.name + (".ogg" if self.type == ".flac" else self.type),
        )
        ...
        if (track_info[1] != "-" and not "-" in track_info[0]) or (
            not "-" in track_info[0]
        ):  # test for 1-12 style
            # means [0] is `12.` or `12`
        ...
        with os.scandir(path) as it:
            ...
            for entry in it:
                if entry.is_file() and (
                    ...
                    or entry.name.endswith(".flac")
                    or entry.name.endswith(".ogg")
                ):
                    songs.append(
                        Song(entry.name, entry.path, artist, name, destination)
                    )
