...
if __name__ == "__main__":
    ...
    destination = (
        input(f"Path to mounted usb (default: {default_destination}): ")
    ...
    if user_input_yn("Remove existing? (Y/n) "):
        ...
        user_input_list(
            list(existing),
            "Enter the numbers of the artists you want to remove: (comma separated, no spaces. ie 1,2,3 q to quit) ",
            queue["delete_artists"],
            page_size,
        )

    if user_input_yn("Add new? (Y/n) "):
        candidates = filter(
            lambda x: (x.remove_artist or not x.dest_artist_full), artist_list
        )
        selected = user_input_list(
            list(candidates),
            "Enter the numbers of the artists you'd like to add: (comma separated, no spaces. ie 1,2,3 q to quit) ",
            [],
            page_size,
        )
        if user_input_yn("Skip individual album selection? (Y/n) "):
            ...
            for artist in selected:
                user_input_list(
                    list(artist.albums),
                    "Enter the numbers of the albums you'd like to add: (comma separated, no spaces. ie 1,2,3 q to quit) ",
                    queue["copy_albums"],
                    page_size,
                )
